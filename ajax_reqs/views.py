...
def employee_ajax(request):
	...
	# return HttpResponse(simplejson.dumps([{'display_name' :user.first_name, 'display_pic' : user.photo.name, 'id' : str(user.id)} for user in data]), mimetype="application/json")
